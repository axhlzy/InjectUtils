...
public:
    ...
    struct Assembly final {
        ...
        [[nodiscard]] auto Get(const std::string &strClass,
                               const std::string &strNamespace = "*",
                               const std::string &strParent = "*") const
            -> Class * {
            for (const auto pClass : classes)
                if (strClass == pClass->name &&
                    (strNamespace == "*" || pClass->namespaze == strNamespace) &&
    ...
    struct Class final {
        ...
        auto Get(const std::string &name, const std::vector<std::string> &args = {})
            -> RType * {
                        ...
                        if (pMethod->args.size() == args.size()) {
                            ...
                            for (size_t i{0}; const auto &typeName : args)
                                if (typeName == "*" || typeName.empty()
                                        ? true
                                        : pMethod->args[i++]->pType->name == typeName) {
                                    index++;
                                }
        ...
        auto GetValue(void *obj, const std::string &name) -> RType {
            return *reinterpret_cast<RType *>(reinterpret_cast<uintptr_t>(obj) +
                                              Get<Field>(name)->offset);
        }

        template <typename RType>
        auto SetValue(void *obj, const std::string &name, RType value) -> void {
            return *reinterpret_cast<RType *>(reinterpret_cast<uintptr_t>(obj) +
                                              Get<Field>(name)->offset) = value;
        }

        [[nodiscard]] auto GetType() const -> Type {
            if (mode_ == Mode::Il2Cpp) {
                const auto pUType =
                    Invoke<void *, void *>("il2cpp_class_get_type", classinfo);
                return {pUType, name, -1};
            }
            const auto pUType =
                Invoke<void *, void *>("mono_class_get_type", classinfo);
            return {pUType, name, -1};
        }
        ...
        auto FindObjectsByType() -> std::vector<T> {
            ...
            if (!pMethod)
                pMethod =
                    UnityResolve::Get("UnityEngine.CoreModule.dll")
                        ->Get("Object")
                        ->Get<Method>(mode_ == Mode::Il2Cpp ? "FindObjectsOfType"
            ...
            if (pMethod) {
                std::vector<T> rs{};
                auto array =
                    pMethod->Invoke<UnityType::Array<T> *>(this->GetType().GetObject());
                rs.reserve(array->max_length);
                for (auto i = 0; i < array->max_length; i++)
        ...
        auto New() -> T * {
            if (mode_ == Mode::Il2Cpp)
                return Invoke<T *, void *>("il2cpp_object_new", classinfo);
    ...
    struct Field final {
        ...
        auto SetValue(T *value) const -> void {
            if (!static_field)
                return;
            if (mode_ == Mode::Il2Cpp)
                return Invoke<void, void *, T *>("il2cpp_field_static_set_value",
                                                 fieldinfo, value);
        }
        ...
        auto GetValue(T *value) const -> void {
            if (!static_field)
                return;
            if (mode_ == Mode::Il2Cpp)
                return Invoke<void, void *, T *>("il2cpp_field_static_get_value",
                                                 fieldinfo, value);
        }
    };
    ...
    public:
        ...
        auto Invoke(Args... args) -> Return {
            ...
            try {
                if (!badPtr)
                    badPtr = !IsBadCodePtr(static_cast<FARPROC>(function));
                if (function && badPtr)
                    return reinterpret_cast<Return(UNITY_CALLING_CONVENTION *)(Args...)>(
                        function)(args...);
            } catch (...) {
            }
        ...
        auto RuntimeInvoke(Obj *obj, Args... args) -> Return {
            ...
            if (mode_ == Mode::Il2Cpp) {
                if constexpr (std::is_same_v<Return, void>) {
                    UnityResolve::Invoke<void *>("il2cpp_runtime_invoke", address, obj,
                                                 argArray, exc);
                    return;
                } else
            ...
            if constexpr (std::is_same_v<Return, void>) {
                UnityResolve::Invoke<void *>("mono_runtime_invoke", address, obj,
                                             argArray, exc);
                return;
            } else
                return *static_cast<Return *>(UnityResolve::Invoke<void *>(
                    "mono_runtime_invoke", address, obj, argArray, exc));
        }
    ...
    static auto Init(void *hmodule, const Mode mode = Mode::Mono) -> void {
        ...
        if (mode_ == Mode::Il2Cpp) {
            ...
            if (Get("UnityEngine.dll") && (!Get("UnityEngine.CoreModule.dll") ||
                                           !Get("UnityEngine.PhysicsModule.dll"))) {
                // 兼容某些游戏 (如生死狙击2)
                for (const std::vector<std::string> names =
                    ...
                    const auto assembly = new Assembly{.address = ass->address,
                                                       .name = name,
                                                       .file = ass->file,
                                                       .classes = ass->classes};
        ...
        for (const auto &pAssembly : assembly) {
            for (const auto &pClass : pAssembly->classes) {
                io << std::format("\tnamespace: {}",
                                  pClass->namespaze.empty() ? "" : pClass->namespaze);
                io << "\n";
                io << std::format("\tAssembly: {}\n",
                                  pAssembly->name.empty() ? "" : pAssembly->name);
                io << std::format("\tAssemblyFile: {} \n",
                                  pAssembly->file.empty() ? "" : pAssembly->file);
                io << std::format("\tclass {}{} ", pClass->name,
                                  pClass->parent.empty() ? "" : " : " + pClass->parent);
                io << "{\n\n";
                for (const auto &pField : pClass->fields)
                    io << std::format("\t\t{:+#06X} | {}{} {};\n", pField->offset,
                                      pField->static_field ? "static " : "",
                                      pField->type->name, pField->name);
                io << "\n";
                for (const auto &pMethod : pClass->methods) {
                    io << std::format(
                        "\t\t[Flags: {:032b}] [ParamsCount: {:04d}] |RVA: {:+#010X}|\n",
                        pMethod->flags, pMethod->args.size(),
                        reinterpret_cast<std::uint64_t>(pMethod->function) -
                            reinterpret_cast<std::uint64_t>(hmodule_));
                    io << std::format("\t\t{}{} {}(",
                                      pMethod->static_function ? "static " : "",
                                      pMethod->return_type->name, pMethod->name);
                    std::string params{};
        ...
        for (const auto &pAssembly : assembly) {
            for (const auto &pClass : pAssembly->classes) {
                ...
                io2 << std::format("\tAssemblyFile: {} \n",
                                   pAssembly->file.empty() ? "" : pAssembly->file);
                io2 << std::format("\tstruct {}{} ", pClass->name,
                                   pClass->parent.empty() ? ""
                                                          : " : " + pClass->parent);
                io2 << "{\n\n";
                ...
                next:
                    ...
                    if (field->type->name == "System.Int64") {
                        ...
                        if (!pClass->fields[i + 1]->static_field &&
                            (pClass->fields[i + 1]->offset - field->offset) > 8) {
                    ...
                    if (field->type->name == "System.Int32") {
                        io2 << std::format("\t\tint {};\n", name);
                        if (!pClass->fields[i + 1]->static_field &&
                            (pClass->fields[i + 1]->offset - field->offset) > 4) {
                            io2 << std::format("\t\tchar {}_[0x{:06X}];\n", name,
                                               pClass->fields[i + 1]->offset - field->offset -
                                                   4);
                        }
                    ...
                    if (field->type->name == "System.Boolean") {
                        io2 << std::format("\t\tbool {};\n", name);
                        if (!pClass->fields[i + 1]->static_field &&
                            (pClass->fields[i + 1]->offset - field->offset) > 1) {
                            io2 << std::format("\t\tchar {}_[0x{:06X}];\n", name,
                                               pClass->fields[i + 1]->offset - field->offset -
                                                   1);
                        }
    ...
    static auto Invoke(const std::string &funcName, Args... args) -> Return {
        ...
        if (address_.find(funcName) == address_.end() ||
            address_[funcName] == nullptr) {
#if WINDOWS_MODE
            address_[funcName] = static_cast<void *>(
                GetProcAddress(static_cast<HMODULE>(hmodule_), funcName.c_str()));
#elif ANDROID_MODE || LINUX_MODE
...
private:
    static auto ForeachAssembly() -> void {
        ...
        if (mode_ == Mode::Il2Cpp) {
            ...
            const auto assemblies = Invoke<void **>("il2cpp_domain_get_assemblies",
                                                    pDomain, &nrofassemblies);
            for (auto i = 0; i < nrofassemblies; i++) {
                ...
                assembly->file =
                    Invoke<const char *>("il2cpp_image_get_filename", image);
                assembly->name = Invoke<const char *>("il2cpp_image_get_name", image);
                UnityResolve::assembly.push_back(assembly);
                ForeachClass(assembly, image);
            }
        } else {
            Invoke<void *, void (*)(void *ptr, std::vector<Assembly *> &),
                   std::vector<Assembly *> &>(
                ...
                [](void *ptr, std::vector<Assembly *> &v) {
                    ...
                    assembly->file =
                        Invoke<const char *>("mono_image_get_filename", image);
    ...
    static auto ForeachClass(Assembly *assembly, void *image) -> void {
        ...
        if (mode_ == Mode::Il2Cpp) {
            ...
            for (auto i = 0; i < count; i++) {
                ...
                if (const auto pPClass =
                        Invoke<void *>("il2cpp_class_get_parent", pClass))
                    pAClass->parent =
                        Invoke<const char *>("il2cpp_class_get_name", pPClass);
                pAClass->namespaze =
                    Invoke<const char *>("il2cpp_class_get_namespace", pClass);
                ...
                void *iter{};
                do {
                    if ((i_class = Invoke<void *>("il2cpp_class_get_interfaces", pClass,
                                                  &iter))) {
            ...
            for (auto i = 0; i < count; i++) {
                const auto pClass =
                    Invoke<void *>("mono_class_get", image, 0x02000000 | (i + 1));
                if (pClass == nullptr)
                    continue;
                ...
                if (const auto pPClass =
                        Invoke<void *>("mono_class_get_parent", pClass))
                    pAClass->parent =
                        Invoke<const char *>("mono_class_get_name", pPClass);
                pAClass->namespaze =
                    Invoke<const char *>("mono_class_get_namespace", pClass);
                assembly->classes.push_back(pAClass);
                ...
                do {
                    if ((iClass = Invoke<void *>("mono_class_get_interfaces", pClass,
                                                 &iiter))) {
    ...
    static auto ForeachFields(Class *klass, void *pKlass) -> void {
        ...
        if (mode_ == Mode::Il2Cpp) {
            ...
            do {
                if ((field =
                         Invoke<void *>("il2cpp_class_get_fields", pKlass, &iter))) {
                    const auto pField = new Field{
                        .fieldinfo = field,
                        .name = Invoke<const char *>("il2cpp_field_get_name", field),
                        .type = new Type{.address = Invoke<void *>(
                    ...
                    pField->type->name = Invoke<const char *>("il2cpp_type_get_name",
                                                              pField->type->address);
                    pField->type->size = -1;
            ...
            do {
                if ((field = Invoke<void *>("mono_class_get_fields", pKlass, &iter))) {
                    const auto pField = new Field{
                        ...
                        .type = new Type{.address = Invoke<void *>("mono_field_get_type",
                                                                   field)},
                        .klass = klass,
                    ...
                    pField->static_field = pField->offset <= 0;
                    pField->type->name =
                        Invoke<const char *>("mono_type_get_name", pField->type->address);
                    pField->type->size =
                        Invoke<int>("mono_type_size", pField->type->address, &tSize);
                    klass->fields.push_back(pField);
                }
            } while (field);
        }
    ...
    static auto ForeachMethod(Class *klass, void *pKlass) -> void {
        ...
        if (mode_ == Mode::Il2Cpp) {
            ...
            do {
                if ((method =
                         Invoke<void *>("il2cpp_class_get_methods", pKlass, &iter))) {
                    int fFlags{};
                    const auto pMethod = new Method{};
                    pMethod->address = method;
                    pMethod->name =
                        Invoke<const char *>("il2cpp_method_get_name", method);
                    pMethod->klass = klass;
                    pMethod->return_type = new Type{
                        .address =
                            Invoke<void *>("il2cpp_method_get_return_type", method),
                    };
                    pMethod->flags =
                        Invoke<int>("il2cpp_method_get_flags", method, &fFlags);

                    int tSize{};
                    pMethod->static_function = pMethod->flags & 0x10;
                    pMethod->return_type->name = Invoke<const char *>(
                        "il2cpp_type_get_name", pMethod->return_type->address);
                    pMethod->return_type->size = -1;
                    pMethod->function = *static_cast<void **>(method);
                    klass->methods.push_back(pMethod);
                    const auto argCount =
                        Invoke<int>("il2cpp_method_get_param_count", method);
                    for (auto index = 0; index < argCount; index++)
                        pMethod->args.push_back(new Method::Arg{
                            Invoke<const char *>("il2cpp_method_get_param_name", method,
                                                 index),
                            new Type{.address = Invoke<void *>("il2cpp_method_get_param",
                                                               method, index),
                                     .name = Invoke<const char *>(
                                         "il2cpp_type_get_name",
                                         Invoke<void *>("il2cpp_method_get_param", method,
                                                        index)),
                                     .size = -1}});
                }
            } while (method);
            ...
            do {
                if ((method =
                         Invoke<void *>("mono_class_get_methods", pKlass, &iter))) {
                    const auto signature =
                        Invoke<void *>("mono_method_signature", method);
                    int fFlags{};
                    const auto pMethod = new Method{};
                    pMethod->address = method;
                    ...
                    pMethod->return_type = new Type{
                        .address =
                            Invoke<void *>("mono_signature_get_return_type", method),
                    };
                    pMethod->flags =
                        Invoke<int>("mono_method_get_flags", method, &fFlags);
                    int tSize{};
                    pMethod->static_function = pMethod->flags & 0x10;
                    pMethod->return_type->name = Invoke<const char *>(
                        "mono_type_get_name", pMethod->return_type->address);
                    pMethod->return_type->size = Invoke<int>(
                        "mono_type_size", pMethod->return_type->address, &tSize);
                    klass->methods.push_back(pMethod);

                    const auto names = new char
                        *[Invoke<int>("mono_signature_get_param_count", signature)];
                    ...
                    auto iname = 0;
                    do {
                        if ((mType = Invoke<void *>("mono_signature_get_params", signature,
                                                    &mIter))) {
                            int t_size{};
                            pMethod->args.push_back(new Method::Arg{
                                names[iname],
...
public:
    ...
    public:
        ...
        struct Vector3 {
            ...
            auto ToVectors(Vector3 *m_pForward, Vector3 *m_pRight,
                           Vector3 *m_pUp) const -> void {
                ...
                if (m_pRight) {
                    ...
                    m_pRight->z =
                        -1.f * m_fSinZ * m_fSinX * m_fSinY + -1.f * m_fCosZ * m_fCosY;
                }
        ...
        struct Quaternion {
            ...
            Quaternion(const float f1, const float f2, const float f3,
                       const float f4) {
                x = f1;
                y = f2;
                z = f3;
            ...
            auto Euler(const Vector3 &m_vRot) -> Quaternion {
            ...
            [[nodiscard]] auto ToEuler() const -> Vector3 {
                ...
                if (const auto m_fTest = x * w - y * z; m_fTest > 0.4995F * m_fDist) {
                    ...
                    m_vEuler.y =
                        atan2f(2.F * w * y + 2.F * z * x, 1.F - 2.F * (x * x + y * y));
                    m_vEuler.z =
                        atan2f(2.F * w * z + 2.F * x * y, 1.F - 2.F * (z * z + x * x));
                }
        ...
        struct Color {
            ...
            explicit Color(const float fRed = 0.f, const float fGreen = 0.f,
                           const float fBlue = 0.f, const float fAlpha = 1.f) {
                r = fRed;
                g = fGreen;
                b = fBlue;
                a = fAlpha;
        ...
        struct Object {
            ...
            auto GetType() -> CsType * {
                ...
                if (!method)
                    method = Get("mscorlib.dll")->Get("Object")->Get<Method>("GetType");
                if (method)
            ...
            auto ToString() -> std::string {
                static Method *method;
                if (!method)
                    method = Get("mscorlib.dll")->Get("Object")->Get<Method>("ToString");
                if (method)
        ...
        struct CsType {
            auto FormatTypeName() -> std::string {
                ...
                if (!method)
                    method =
                        Get("mscorlib.dll")->Get("Type")->Get<Method>("FormatTypeName");
                if (method)
                    return method->Invoke<String *>(this)->ToString();
                throw std::logic_error("nullptr");
            }
            ...
            auto GetFullName() -> std::string {
                ...
                if (!method)
                    method =
                        Get("mscorlib.dll")->Get("Type")->Get<Method>("get_FullName");
                if (method)
                    return method->Invoke<String *>(this)->ToString();
                throw std::logic_error("nullptr");
            }
            ...
            auto GetNamespace() -> std::string {
                ...
                if (!method)
                    method =
                        Get("mscorlib.dll")->Get("Type")->Get<Method>("get_Namespace");
                if (method)
                    return method->Invoke<String *>(this)->ToString();
                throw std::logic_error("nullptr");
            }
        ...
        struct String : Object {
            ...
            [[nodiscard]] auto ToString() const -> std::string {
                ...
                WideCharToMultiByte(CP_UTF8, 0, m_firstChar, m_stringLength,
                                    sRet.data(), static_cast<int>(sRet.size()), nullptr,
                                    nullptr);
                return sRet;
#elif LINUX_MODE
            ...
            static auto New(const std::string &str) -> String * {
                if (mode_ == Mode::Il2Cpp)
                    return UnityResolve::Invoke<String *, const char *>(
                        "il2cpp_string_new", str.c_str());
                return UnityResolve::Invoke<String *, void *, const char *>(
                    "mono_string_new",
                    UnityResolve::Invoke<void *>("mono_get_root_domain"), str.c_str());
            }
        };
        ...
        struct Array : Object {
            ...
            auto Insert(T *m_pArray, uintptr_t m_uSize, const uintptr_t m_uIndex = 0)
                -> void {
            ...
            auto RemoveAt(const unsigned int m_uIndex) -> void {
                ...
                if (max_length > (m_uIndex + 1))
                    for (auto u = m_uIndex;
                         (static_cast<unsigned int>(max_length) - m_uIndex) > u; ++u)
                        operator[](u) = operator[](u + 1);

                --max_length;
            }

            auto RemoveRange(const unsigned int m_uIndex, unsigned int m_uCount)
                -> void {
                if (m_uCount == 0)
                    m_uCount = 1;
                ...
                if (max_length > (m_uTotal + 1))
                    for (auto u = m_uIndex;
                         (static_cast<unsigned int>(max_length) - m_uTotal) >= u; ++u)
            ...
            static auto New(const Class *kalss, const std::uintptr_t size)
                -> Array * {
                if (mode_ == Mode::Il2Cpp)
                    return UnityResolve::Invoke<Array *, void *, std::uintptr_t>(
                        "il2cpp_array_new", kalss->classinfo, size);
                return UnityResolve::Invoke<Array *, void *, void *, std::uintptr_t>(
                    "mono_array_new", pDomain, kalss->classinfo, size);
            }
        };
        ...
        struct List : Object {
            ...
            auto Add(Type *pDate) -> float {
                static Method *method;
                if (!method)
                    method = Get("mscorlib.dll")->Get("List")->Get<Method>("Add");
                if (method)
            ...
            auto Remove(Type *pDate) -> float {
                static Method *method;
                if (!method)
                    method = Get("mscorlib.dll")->Get("List")->Get<Method>("Remove");
                if (method)
                    return method->Invoke<void>(this, pDate);
                throw std::logic_error("nullptr");
            }
            ...
            auto RemoveAt(int index) -> float {
                static Method *method;
                if (!method)
                    method = Get("mscorlib.dll")->Get("List")->Get<Method>("RemoveAt");
                if (method)
            ...
            auto ForEach(void (*action)(Type *pDate)) -> float {
                static Method *method;
                if (!method)
                    method = Get("mscorlib.dll")->Get("List")->Get<Method>("ForEach");
                if (method)
            ...
            auto GetRange(int index, int count) -> float {
                static Method *method;
                if (!method)
                    method = Get("mscorlib.dll")->Get("List")->Get<Method>("GetRange");
                if (method)
                    return method->Invoke<void>(this, index, count);
                throw std::logic_error("nullptr");
            }
            ...
            auto Clear() -> float {
                static Method *method;
                if (!method)
                    method = Get("mscorlib.dll")->Get("List")->Get<Method>("Clear");
                if (method)
            ...
            auto Sort(int (*comparison)(Type *pX, Type *pY)) -> float {
                static Method *method;
                if (!method)
                    method = Get("mscorlib.dll")->Get("List")->Get<Method>("Sort");
                if (method)
        ...
        struct Dictionary : Object {
            ...
            auto GetEntry() -> Entry * {
                return static_cast<Entry *>(pEntries->GetData());
            }
        ...
        struct UnityObject : Object {
            ...
            auto GetName() -> std::string {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Object")
                                 ->Get<Method>("get_name");
                if (method)
                    return method->Invoke<String *>(this)->ToString();
            ...
            static auto ToString(UnityObject *obj) -> std::string {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ...
                                 ->Get<Method>("ToString", {"*"});
                if (method)
                    return method->Invoke<String *>(obj)->ToString();
                throw std::logic_error("nullptr");
            }
        ...
        struct Component : UnityObject {
            ...
            auto GetComponentsInChildren() -> Array<T> * {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Component")
                                 ->Get<Method>("GetComponentsInChildren");
                if (method)
                    return method->Invoke<Array<T> *>(this);
                throw std::logic_error("nullptr");
            }
            ...
            auto GetComponentsInChildren(Class *pClass) -> Array<T> * {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Component")
                            ->Get<Method>("GetComponentsInChildren", {"System.Type"});
                ;
                if (method)
                    return method->Invoke<Array<T> *>(this,
                                                      pClass->GetType().GetObject());
                throw std::logic_error("nullptr");
            }
            ...
            auto GetComponents(Class *pClass) -> Array<T> * {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Component")
                                 ->Get<Method>("GetComponents", {"System.Type"});
                ;
                if (method)
                    return method->Invoke<Array<T> *>(this,
                                                      pClass->GetType().GetObject());
            ...
            auto GetComponentsInParent() -> Array<T> * {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Component")
                                 ->Get<Method>("GetComponentsInParent");
                if (method)
                    return method->Invoke<Array<T> *>(this);
                throw std::logic_error("nullptr");
            }
            ...
            auto GetComponentsInParent(Class *pClass) -> Array<T> * {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Component")
                                 ->Get<Method>("GetComponentsInParent", {"System.Type"});
                ;
                if (method)
                    return method->Invoke<Array<T> *>(this,
                                                      pClass->GetType().GetObject());
            ...
            auto GetComponentInChildren(Class *pClass) -> T {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Component")
                                 ->Get<Method>("GetComponentInChildren", {"System.Type"});
                ;
                if (method)
                    return method->Invoke<T>(this, pClass->GetType().GetObject());
                throw std::logic_error("nullptr");
            }
            ...
            auto GetComponentInParent(Class *pClass) -> T {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Component")
                                 ->Get<Method>("GetComponentInParent", {"System.Type"});
                ;
                if (method)
                    return method->Invoke<T>(this, pClass->GetType().GetObject());
                throw std::logic_error("nullptr");
        ...
        struct Camera : Component {
            ...
            static auto GetAllCount() -> int {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Camera")
                                 ->Get<Method>("get_allCamerasCount");
                if (method)
            ...
            auto SetDepth(const float depth) -> void {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Camera")
                                 ->Get<Method>("set_depth", {"*"});
                if (method)
                    return method->Invoke<void>(this, depth);
            }

            auto WorldToScreenPoint(const Vector3 &position, const Eye eye)
                -> Vector3 {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Camera")
                                 ->Get<Method>(mode_ == Mode::Mono
            ...
            auto ScreenToWorldPoint(const Vector3 &position, const Eye eye)
                -> Vector3 {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
            ...
            auto GetPosition() -> Vector3 {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Transform")
                            ->Get<Method>(mode_ == Mode::Mono ? "get_position_Injected"
                                                              : "get_position");
                if (mode_ == Mode::Mono && method) {
                    const Vector3 vec3{};
            ...
            auto SetPosition(const Vector3 &position) -> void {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Transform")
                            ->Get<Method>(mode_ == Mode::Mono ? "set_position_Injected"
                                                              : "set_position");
                if (mode_ == Mode::Mono && method)
            ...
            auto GetRotation() -> Quaternion {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Transform")
                            ->Get<Method>(mode_ == Mode::Mono ? "get_rotation_Injected"
                                                              : "get_rotation");
                if (mode_ == Mode::Mono && method) {
            ...
            auto SetRotation(const Quaternion &position) -> void {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Transform")
                            ->Get<Method>(mode_ == Mode::Mono ? "set_rotation_Injected"
                                                              : "set_rotation");
                if (mode_ == Mode::Mono && method)
                    return method->Invoke<void>(this, &position);
                if (method)
            ...
            auto GetLocalScale() -> Vector3 {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Transform")
                            ->Get<Method>(mode_ == Mode::Mono ? "get_localScale_Injected"
                                                              : "get_localScale");
                if (mode_ == Mode::Mono && method) {
                    const Vector3 vec3{};
                    method->Invoke<void>(this, &vec3);
            ...
            auto SetLocalScale(const Vector3 &position) -> void {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Transform")
                            ->Get<Method>(mode_ == Mode::Mono ? "set_localScale_Injected"
                                                              : "set_localScale");
                if (mode_ == Mode::Mono && method)
                    return method->Invoke<void>(this, &position);
                if (method)
            ...
            auto GetLossyScale() -> Vector3 {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Transform")
                            ->Get<Method>(mode_ == Mode::Mono ? "get_lossyScale_Injected"
                                                              : "get_lossyScale");
                if (mode_ == Mode::Mono && method) {
                    const Vector3 vec3{};
                    method->Invoke<void>(this, &vec3);
            ...
            auto TransformPoint(const Vector3 &position) -> Vector3 {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.CoreModule.dll")
                            ->Get("Transform")
                            ->Get<Method>(mode_ == Mode::Mono ? "TransformPoint_Injected"
                                                              : "TransformPoint");
                if (mode_ == Mode::Mono && method) {
                    const Vector3 vec3{};
        ...
        struct GameObject : UnityObject {
            static auto Create(GameObject *obj, const std::string &name) -> void {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("GameObject")
                                 ->Get<Method>("Internal_CreateGameObject");
                if (method)
                    method->Invoke<void, GameObject *, String *>(obj, String::New(name));
                throw std::logic_error("nullptr");
            }

            static auto FindGameObjectsWithTag(const std::string &name)
                -> std::vector<GameObject *> {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("GameObject")
                                 ->Get<Method>("FindGameObjectsWithTag");
                if (method) {
                    std::vector<GameObject *> rs{};
                    const auto array =
                        method->Invoke<Array<GameObject *> *>(String::New(name));
                    rs.reserve(array->max_length);
                    for (auto i = 0; i < array->max_length; i++)
                        rs.push_back(array->At(i));
                    return rs;
            ...
            auto GetComponent(const Class *type) -> T {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("GameObject")
                                 ->Get<Method>("GetComponent", {"System.Type"});
                if (method)
                    return method->Invoke<T>(this, type->GetType().GetObject());
                throw std::logic_error("nullptr");
            }
            ...
            auto GetComponents(Class *type,
                               ...
                               bool recursive = false, bool includeInactive = true,
                               bool reverse = false, List<T> *resultList = nullptr)
                -> std::vector<T> {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("GameObject")
                                 ->Get<Method>("GetComponentsInternal");
                if (method)
                    return method
                        ->Invoke<Array<T> *>(this, type->GetType().GetObject(),
                                             useSearchTypeAsArrayReturnType, recursive,
            ...
            template <typename T>
            auto GetComponentsInChildren(Class *type,
                                         const bool includeInactive = false)
                -> std::vector<T> {
                return GetComponents<T>(type, false, true, includeInactive, false,
                                        nullptr);
            }

            template <typename T>
            auto GetComponentsInParent(Class *type,
                                       const bool includeInactive = false)
                -> std::vector<T> {
                return GetComponents<T>(type, false, true, includeInactive, true,
                                        nullptr);
            }
        };
        ...
        struct Rigidbody : Component {
            auto GetDetectCollisions() -> bool {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.PhysicsModule.dll")
                                 ->Get("Rigidbody")
                                 ->Get<Method>("get_detectCollisions");
                if (method)
                    return method->Invoke<bool>(this);
                throw std::logic_error("nullptr");
            }
            ...
            auto SetDetectCollisions(const bool value) -> void {
                ...
                if (!method)
                    method = Get("UnityEngine.PhysicsModule.dll")
                                 ->Get("Rigidbody")
                                 ->Get<Method>("set_detectCollisions");
                if (method)
            ...
            auto GetVelocity() -> Vector3 {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.PhysicsModule.dll")
                            ->Get("Rigidbody")
                            ->Get<Method>(mode_ == Mode::Mono ? "get_velocity_Injected"
                                                              : "get_velocity");
                if (mode_ == Mode::Mono && method) {
                    Vector3 vector;
            ...
            auto SetVelocity(Vector3 value) -> void {
                ...
                if (!method)
                    method =
                        Get("UnityEngine.PhysicsModule.dll")
                            ->Get("Rigidbody")
                            ->Get<Method>(mode_ == Mode::Mono ? "set_velocity_Injected"
                                                              : "set_velocity");
                if (mode_ == Mode::Mono && method)
                    return method->Invoke<void>(this, &value);
                if (method)
        ...
        struct Collider : Component {
            auto GetBounds() -> Bounds {
                ...
                if (!method)
                    method = Get("UnityEngine.PhysicsModule.dll")
                                 ->Get("Collider")
                                 ->Get<Method>("get_bounds_Injected");
                if (method) {
                    Bounds bounds;
        ...
        struct Behaviour : Component {
            ...
            auto SetEnabled(const bool value) -> bool {
                ...
                if (!method)
                    method = Get("UnityEngine.CoreModule.dll")
                                 ->Get("Behaviour")
                                 ->Get<Method>("set_enabled");
                if (method)
                    return method->Invoke<bool>(this, value);
                throw std::logic_error("nullptr");
            }
        };
        ...
        struct Physics : Object {
            ...
            static auto Raycast(const Vector3 &origin, const Vector3 &direction,
                                const float maxDistance) -> bool {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.PhysicsModule.dll")
                                 ->Get("Physics")
            ...
            static auto IgnoreCollision(Collider *collider1, Collider *collider2)
                -> void {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.PhysicsModule.dll")
                                 ->Get("Physics")
                                 ->Get<Method>("IgnoreCollision1", {"*", "*"});
                if (method)
        ...
        struct Animator : Behaviour {
            ...
            auto GetBoneTransform(const HumanBodyBones humanBoneId) -> Transform * {
                static Method *method;
                if (!method)
                    method = Get("UnityEngine.AnimationModule.dll")
                                 ->Get("Animator")
                                 ->Get<Method>("GetBoneTransform");
                if (method)
