...
namespace Gum {
    ...
    typedef std::function<void(Gum::InvocationContext *)> LambdaFunc;

    struct Interceptor : public Object {
        virtual bool attach(void *function_address, InvocationListener *listener,
                            void *listener_function_data = 0) = 0;

        virtual bool
        attach(void *function_address, LambdaFunc on_enter, LambdaFunc on_leave = nullptr,
               void *listener_function_data = 0);

        virtual void detach(InvocationListener *listener) = 0;

        virtual void
        replace(void *function_address, void *replacement_address, void *replacement_data = 0) = 0;

        virtual void
        replace(void *function_address, void *replacement_address, void *replacement_data,
    ...
    struct Backtracer : public Object {
        virtual void
        generate(const CpuContext *cpu_context, ReturnAddressArray &return_addresses) const = 0;
    };
    ...
    GUMPP_CAPI bool
    ReturnAddressDetails_from_address(ReturnAddress address, ReturnAddressDetails &details);
    ...
    struct HeapApi {
        ...
        void *(*_calloc_dbg)(size_t num, size_t size, int block_type, const char *filename,
                             int linenumber);

        void *
        (*_realloc_dbg)(void *old_address, size_t new_size, int block_type, const char *filename,
                        int linenumber);
    ...
    struct InspectorCallbacks {
        ...
        virtual void inspect_worst_case(InvocationContext *context, char *output_buf,
                                        unsigned int output_buf_len) = 0;
    };
    ...
    struct Profiler : public Object {
        virtual void instrument_functions_matching(const char *match_str, Sampler *sampler,
                                                   FunctionMatchCallbacks *match_callbacks = 0) = 0;

        virtual bool instrument_function(void *function_address, Sampler *sampler) = 0;

        virtual bool instrument_function_with_inspector(void *function_address, Sampler *sampler,
                                                        InspectorCallbacks *inspector_callbacks) = 0;
    ...
    class LambdaListener : public Gum::InvocationListener {

    public:

        LambdaListener(LambdaFunc on_enter_lambda, LambdaFunc on_leave_lambda)
                : on_enter_lambda_(on_enter_lambda), on_leave_lambda_(on_leave_lambda) {}

        void on_enter(Gum::InvocationContext *context) override {
            // auto ctx = (GumArm64CpuContext*)context->get_cpu_context();
            on_enter_lambda_(context);
        }

        void on_leave(Gum::InvocationContext *context) override {
            on_leave_lambda_(context);
        }

    private:
        LambdaFunc on_enter_lambda_;
        LambdaFunc on_leave_lambda_;
    };
