...
#else
    handle = (*env)->CallStaticLongMethod(env, jniHelper, jniCall, (long) dlopen, (long) filename,
                                          (long) flags);
#endif
    return handle;
}
...
#else
    ptr = (*env)->CallStaticLongMethod(env, jniHelper, jniCall, (long) dlsym, (long) handle,
                                       (long) symbol);
#endif
    return ptr;
}
...
static jclass findHelperClass(JNIEnv *env) {
    ...
    void *classes_dex = NULL;
    jlong classes_dex_len = 0;
...
//        jclass localClass = (*env)->FindClass(env, "lab/galaxy/dlfunc/JNIHelper");
        ...
        if (rc != JNI_OK) {
            LOGE("failed to register native method %s%s at %p", METHOD_NAME, METHOD_SIG,
                 METHOD_PTR);
            (*env)->ExceptionClear(env);
            return rc;
        }
