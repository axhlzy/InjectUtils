...
public:
    Tick(Replxx &replxx_, std::string const &keys_, bool tickMessages_, bool promptFan_)
        : _thread(), _tick(0), _promptState(0), _alive(false), _keys(keys_.begin(), keys_.end()), _tickMessages(tickMessages_), _promptFan(promptFan_), _replxx(replxx_) {
    }
    void start() {
        _alive = true;
...
int installRepl(const std::vector<std::string> &suggestions, std::function<void(const std::string &)> callback) {

    std::vector<std::string> l_suggestions{
        ".help",
        ".history",
        ".quit",
        ".exit",
    ...
    l_suggestions.insert(l_suggestions.end(), suggestions.begin(), suggestions.end());
    ...
    rx.set_completion_callback(std::bind(&hook_completion, _1, _2, cref(l_suggestions), ignoreCase));
    rx.set_highlighter_callback(std::bind(&hook_color, _1, _2, cref(regex_color), cref(word_color)));
    rx.set_hint_callback(std::bind(&hook_hint, _1, _2, _3, cref(l_suggestions), ignoreCase));
    if (promptInCallback) {
